...
namespace jrmwng
{
	...
	struct sudoku_t
	{
		...
		long set_candidates(unsigned uX, unsigned uY, long lCandidateSet)
		{
			auto & stNumber = astNumber[uX + uY * TT::SUDOKU_CANDIDATE_COUNT];

			if (stNumber.lCandidateSet != lCandidateSet)
			{
				stNumber.lCandidateSet = lCandidateSet;
				return stNumber.lGroupSet;
			}
			return 0;
		}
		long get_candidates(unsigned uX, unsigned uY) const
		{
			return astNumber[uX + uY * TT::SUDOKU_CANDIDATE_COUNT].lCandidateSet;
		}
	};
}
